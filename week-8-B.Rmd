...
$$ \textit{att_chg}_i = \beta_1 x_{t,i} + \beta_2 x_{c,i} + \epsilon_i  $$
$$ att\_chg_i = \beta_1 x_{t,i} + \beta_2 x_{c,i} + \epsilon_i  $$

* Describe the ideal Tom Hanks Table.

- no missing values: we would have every person's attitude change for the control
and treatment options
- columns: person/subject, initial attitude, attitude after control, attitude
after treatment, change in attitude for control, change in attitude for treatment

* Write a few sentences about whether or not our data will allow us to answer this question.

- if we knew gender, party, liberal of preceptor/tom hanks, then we might be 
able to predict his response
...
fit_obj %>%
  ...
  ggplot(aes(x = treatmentTreated)) + 
    ...
    labs(title = "Posterior Probability Distribution",
         subtitle = "Expected Attitude Change for Tom Hanks",
         x = "Attitude Change",
         y = "Probability") + 
...
pp %>%
  ...
  ggplot(aes(x = THanks)) +
    ...
    labs(title = "Posterior Predictive Distribution",
         subtitle = "For Tom Hank's Change in Immigration Attitude",
